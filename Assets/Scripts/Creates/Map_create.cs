...
public class Map_create : MonoBehaviour
{
    ...
    static void Load_BL(int id, int pos, float y, string name, int q_pi, int hei, int fst, int id_cnt, int to_ground)
    {
    ...
    static void BKwood_create()
    {
        for(int f = 1; f <= floor_cnt; f++)
        {
            for(int j = 1; j < room_cnt; j++)
            {
                ...
                int pos = j;
                int q_pi = (int)(Rand.rand() % 3 + 1);
                Load_BL(id, pos, 10000, "flower", q_pi, 17, 1, 3, -3);
                Load_BL(id, pos, 10000, "grass", q_pi, 17, 1, 5, -3);
            }
        }
    }
}
